  ...
  describe("thunks", () => {
    describe("checkoutCart w/mocked dispatch", () => {
        it("should checkout", async () => {
            ...
            expect(calls[0][0].type).toEqual("cart/checkout/fulfilled");
            expect(calls[0][0].type).toEqual({ sucess: true});
        ...
        it("should fail with no items", async () => {
            ...
            expect(calls[0][0].type).toEqual("cart/checkout/pending");
            console.log(calls[0], calls[1])
            expect(calls[0][0].type).toEqual("cart/checkout/rejected");
            expect(calls[0][0].type).toEqual({ sucess: true});
    ...
    describe("checkoutCart w/mock redux store", () => {
        it("should checkout", async () => {
            ...
            expect(actions[1].type).toEqual({ sucess: true});
        });
        it("should fail with no items", async () => {
            const store = mockStore({ cart: { items: { testItem: 3}}});
            await store.dispatch(checkoutCart() as any);
            ...
            expect(actions[1].type).toEqual(undefined);
    ...
    describe("thunks", () => {
        describe("checkoutCart w/full redux store", () => {
          ...
          it("should handle an error message", async () => {
